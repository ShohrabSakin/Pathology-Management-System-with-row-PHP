...
<body style="background-color:lightyellow;font-weight:bold;">
    ...
    <div class="container-fluid">
        ...
        <h1 class="text-center" style="color:blue;">H/O Items</h1> <hr/> <hr/>
        <?php
...
<script>
    ...
    function display(){
           ...
           // let p2 = p1.then(res =>res.json()).then(data => {
            $.get(`${U}/getH_O.php`,{},function(data){
data=JSON.parse(data)
               ...
               $("#tot").html(data.total);
                ...
                //for(d of st.records){  
                    ...
                    a+=`<tr onclick="show?.('${d?.id}','${d?.name}','${d?.year}','${i}')"><td>${d.id}</td><td>${d.name}</td><td>${d.year}</td><td><button type="button"  class="btn btn-danger" onclick="remove('${d.id}',event)">Remove</button></td></tr>`;
                    i++;//i=i+1;
                } )      
...
b=$("#name").val();
c=$("#year").val(); 
        url=`${U}/insertH_O.php?id=${a}&name=${b}&year=${c}`;   
        console.log(url);
...
c=$("#year").val(); 
url=`${U}/updateH_O.php?id=${a}&name=${b}&year=${c}`;
$.get(url,{},function(data){
work();
});
