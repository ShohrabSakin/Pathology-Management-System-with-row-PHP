<?php
    ...
    if( isset($_GET['vrno']) && isset($_GET['slno']) && isset($_GET['itemcode']) && isset($_GET['itemname']) && isset($_GET['qty']) && isset($_GET['price']) && isset($_GET['total'])  ){
        $a=new library();	
        $m=$a->db;
        $q="update saleslineitems set vrno='{$_GET['vrno']}',slno={$_GET['slno']},itemcode='{$_GET['itemcode']}',itemname='{$_GET['itemname']}',qty={$_GET['qty']},price={$_GET['price']},total={$_GET['total']} where vrno='{$_GET['vrno']}' and slno={$_GET['slno']}";
        $r=$m->query($q);
        if(!$r)	{
